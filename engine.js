...
export function runCombatSim(setup, atkLuck = 'average', defLuck = 'average', nWaves = 100, isBear = false, isOptimizing = false) {
    ...
    while (isAlive(m_cur) && (isBear || isAlive(e_cur))) {
        ...
        [['atk', 'def'], ['def', 'atk']].forEach(([side, target]) => {
            ...
            const sP = (side==='atk' ? atkP : defP);
            const tP = (side==='atk' ? defP : atkP);
            const sC = (side==='atk' ? m_cur : e_cur);
            const tC = (side==='atk' ? e_cur : m_cur);
            const sS = setup[side];
            const tS = setup[target];
            const tf = (side==='atk' ? ef : mf);
            const sL = (side==='atk' ? atkLuck : defLuck);

            // Precision Multiplier: Source's self-buffs * Enemy's debuffs
            const effectiveMult = (side === 'atk') 
                ? (m_data.selfMult * e_data.enemyMult) 
                : (e_data.selfMult * m_data.enemyMult);
    ...
    const finalizeLogs = (side, skillLogs) => {
        ...
        if (interactions.length > 0) combinedLogs.unshift(`<span class="text-slate-300">[Troop Abilities]</span> ${interactions.join(' | ')}`);
...
function processBatches(batches) {
    let totals = {inf:0,cav:0,arc:0}, 
        avgBase = {inf:{atk:0,def:0,leth:0,hp:0},cav:{atk:0,def:0,leth:0,hp:0},arc:{atk:0,def:0,leth:0,hp:0}};
