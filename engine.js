        ...
        [['atk', 'def'], ['def', 'atk']].forEach(([side, target]) => {
            ...
            ['inf', 'cav', 'arc'].forEach(u => {
                ...
                if (u === 'cav' && tC['arc'] > 1 && tf !== 'arc' && weight.t7 > 0) {
                    ...
                    const b_df = ba.def * (1 + (tS.stats.arc_def + tM_obj.star)/100);
                    const b_hp = ba.hp * (1 + tS.stats.arc_hp/100);
                    pending.push({dict: tC, unit: 'arc', amt: (Math.sqrt(sC[u])*sq_min*atk*leth*1.1*abil*bypass*sM)/(b_df*b_hp*100*(tM_obj.units.arc*widget_mult))});
                } else {
