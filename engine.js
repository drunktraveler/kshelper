    ...
    while (isAlive(m_cur) && (isBear || isAlive(e_cur)) && wave < maxWaves) {
        ...
        [['atk', 'def'], ['def', 'atk']].forEach(([side, target]) => {
            ...
            ['inf', 'cav', 'arc'].forEach(u => {
                ...
                const tSht = (p) => getProb(p, side, u+'_abil', 0, sL);
                let abil = 1.0; const w = sP.weights[u];
                if (u==='arc') abil *= (1 + tSht(0.1*w.t7)) * (1 + ((w.tg5?0.3:(w.tg3?0.2:0))*0.5));
                if (u==='cav') abil *= (1 + tSht(w.tg5?0.15:(w.tg3?0.1:0)));
                if (tf==='inf') { if (u==='cav') df *= (1 + (0.1*tP.weights.inf.t7)); abil *= (1 - ((tP.weights.inf.tg5?0.375:(tP.weights.inf.tg3?0.25:0))*0.36)); }
                const kills = (Math.sqrt(sC[u])*sq_min*atk*leth*tm*abil*(wave_s_mult*Math.pow(1.15,3)))/(df*hp*100);
                pending.push({dict: tC, unit: tf, amt: kills}); if (side === 'atk') totalDmg += kills;
            });
        });
