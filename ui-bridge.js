...
// --- INITIALIZATION ---
window.init = () => {
    console.log("Kingshot System Initializing...");
    
    // Sync roster levels with Hero DB
    ...
    if (heroSelect) {
        ...
        Object.keys(HEROES).sort().forEach(n => {
            heroSelect.innerHTML += `<option value="${n}">${n}</option>`;
        });
        heroSelect.onchange = (e) => renderSkillsInModal(e.target.value, activeSlot.index);
    }
...
function gatherSetup() {
    const getS = (s) => { const obj = {}; document.querySelectorAll(`input[data-side="${s}"]`).forEach(i => obj[i.dataset.stat] = parseFloat(i.value)||0); return obj; };
    ...
    return {
        atk: { batches: collect('atk'), stats: getS('atk'), heroes: state.atk.heroes },
        def: { batches: collect('def'), stats: getS('def'), heroes: state.def.heroes }
    };
}
...
window.handleSimulation = async () => {
    ...
    if (mode === 'monte-carlo') {
        ...
        const w = atkWins >= 50 ? 'atk' : 'def';
        rAvg = {
            m_cur: { inf: w==='atk'?batch.reduce((s,r)=>s+r.m_cur.inf,0)/runs:0, cav: w==='atk'?batch.reduce((s,r)=>s+r.m_cur.cav,0)/runs:0, arc: w==='atk'?batch.reduce((s,r)=>s+r.m_cur.arc,0)/runs:0 },
            e_cur: { inf: w==='def'?batch.reduce((s,r)=>s+r.e_cur.inf,0)/runs:0, cav: w==='def'?batch.reduce((s,r)=>s+r.e_cur.cav,0)/runs:0, arc: w==='def'?batch.reduce((s,r)=>s+r.e_cur.arc,0)/runs:0 },
...
window.runOptimizer = (mode) => {
    ...
    else document.getElementById('opt-best-score').innerText = `Best: +${Math.round(best.score).toLocaleString()} survivors`;
...
window.calculateOptimalLineups = () => {
    ...
    [{l:"Rally",c:"off",j:false},{l:"Rally w/ Joiners",c:"off",j:true},{l:"Garrison",c:"def",j:false},{l:"Garrison w/ Joiners",c:"def",j:true}].forEach(s => {
        ...
        card.innerHTML = `<div class="text-[9px] font-black text-blue-400 uppercase mb-3">${s.l}</div><div class="flex gap-2">${best.leaders.map(n=>`<img src="./assets/${n.toLowerCase()}.png" class="w-10 h-10 rounded-full border border-blue-500" title="${n}">`).join('')}</div>${s.j?`<div class="flex gap-1 mt-2 opacity-50">${best.joiners.map(n=>`<img src="./assets/${n.toLowerCase()}.png" class="w-6 h-6 rounded-full border border-slate-700" title="${n}">`).join('')}</div>`:''}<div class="mt-3 text-xl font-black">${best.score.toFixed(3)}x</div>`;
